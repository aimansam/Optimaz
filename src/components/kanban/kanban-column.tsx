      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-semibold tabular-nums', COLUMN_COUNT_STYLE[id])}>
              {tasks.length}
            </span>
          </div>
          <button
            onClick={() => setAddOpen(true)}
            className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-200/80 hover:text-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-300 transition-colors"
          >
        ...
        {/* Tasks */}
        <div ref={setNodeRef} className="flex flex-1 flex-col gap-2 overflow-y-auto px-3 pb-3" style={{ minHeight: 120 }}>

  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#0a0a0f] p-4">
      <div className="w-full max-w-sm">
        ...
        {/* Card */}
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/50 backdrop-blur-xl">

          {step === 'verify' ? (
            /* ── Step 2: verify OTP ── */
            <>
              <div className="mb-6 flex flex-col items-center gap-3 text-center">
                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl border transition-colors ${isExpired ? 'border-red-500/30 bg-red-500/10' : 'border-indigo-500/30 bg-indigo-500/10'}`}>
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    6-digit code sent to <span className="text-slate-200">{email}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    {isExpired ? (
                      ...
                      <span className={`font-semibold tabular-nums ${countdown <= 15 ? 'text-amber-400' : 'text-slate-300'}`}>
                        {fmt(countdown)}
                      </span>
                    )}
                  </div>
                  <div className="h-1 w-full overflow-hidden rounded-full bg-white/10">
                    <div
                      className={`h-1 rounded-full transition-all duration-1000 ${isExpired ? 'bg-red-500' : countdown <= 15 ? 'bg-amber-400' : 'bg-indigo-500'}`}
                ...
                {isExpired ? (
                  <button
                    onClick={resendOTP}
                    disabled={!!loading}
                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm font-medium text-white transition-all hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                  ...
                  <button
                    ...
                    className="flex w-full items-center justify-center rounded-xl bg-indigo-600 px-4 py-3 text-sm font-medium text-white transition-all hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-40"
                  >
            ...
            <>
              ...
              <div className="mb-6 flex flex-col items-center gap-3 text-center">
                <Link href="/" className="group flex flex-col items-center gap-3">
                  <img
                    ...
                    alt="Optimaz"
                    className="h-14 w-14 rounded-2xl shadow-lg shadow-black/40 transition-opacity group-hover:opacity-80"
                  />
                  <h1 className="text-2xl font-bold tracking-tight text-white group-hover:opacity-80 transition-opacity">
                    Optimaz
                  </h1>
                </Link>
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  disabled={!!loading}
                  className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm font-medium text-white transition-all hover:bg-white/10 hover:border-white/20 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  {loading === 'google' ? (
              ...
              <p className="mt-5 text-center text-[11px] text-slate-600">
                ...
                <Link href="/terms" className="text-slate-400 underline underline-offset-2 hover:text-slate-200">
                  Terms
                </Link>
                {' '}and{' '}
                <Link href="/privacy" className="text-slate-400 underline underline-offset-2 hover:text-slate-200">
                  Privacy Policy
                </Link>
              </p>
              ...
              <p className="mt-2 text-center text-[11px] text-slate-600">
                ...
                <Link href="/pricing" className="text-slate-400 underline underline-offset-2 hover:text-slate-200">
                  Join the waitlist
                </Link>
              </p>
            </>
